...
class MusicPlugin(Star, FileSenderMixin):
    def __init__(self, context: Context, config: AstrBotConfig = None):
        ...
        你是音乐需求分析助手，需严格完成两个任务：1. 提取歌名 2. 判断意图。
         1. 歌名提取：从用户输入中精确识别并提取歌曲名称。用户有可能使用书名号《》，也可能直接说出歌名。提取时需忽略"的"、"一首"、"个"等停用词。若未提及任何歌名，输出"无歌名"。特别注意：歌名中可能包含特殊字符如"-"、"("、")"等，这些是歌名的一部分，不应作为意图判断依据。
        ...
        特别注意：当用户输入中包含"发送"、"语音"等词语时，需要区分这些词是用户表达意图的关键词还是歌名的一部分。例如"发送近藤佑輔的EmA (-狂-)用语音播放"，其中：
        - "发送"、"语音"、"播放"是用户表达意图的关键词
        - 歌名是"EmA (-狂-)"（包含特殊字符"-"、"("、")"）
        - 意图是"发语音"
        
        示例：
        1. 用户输入"我想听《晴天》" → 歌名：晴天；意图：默认
        2. 用户输入"发《孤勇者》的链接给我" → 歌名：孤勇者；意图：发链接
        3. 用户输入"把《小幸运》当文件发过来" → 歌名：小幸运；意图：发文件
        4. 用户输入"发送近藤佑輔的EmA (-狂-)用语音播放" → 歌名：EmA (-狂-)；意图：发语音
        5. 用户输入"今天天气真好" → 歌名：无歌名；意图：无
        """
        # 添加一个配置项，控制是否只在被@时响应
    ...
    async def judge_music_intent(self, text: str) -> tuple[str, str]:
        ...
        try:
            ...
            if "歌名：" in response_text:
                # 提取歌名部分，排除意图部分
                song_part = response_text.split("歌名：")[-1].split("；")[0].strip()
                # 如果歌名中包含了意图字段，需要进一步处理
                if "意图：" in song_part:
                    song_name = song_part.split("意图：")[0].strip()
                else:
                    song_name = song_part
